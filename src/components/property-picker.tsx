...
export function PropertyPicker({ values, onChange }: { values: string[]; onChange: (values: string[]) => void }) {
  ...
  const matches = (catalog.data ?? [])
    ...
    .filter((item) => !query || normalizePropertyName(item.name).includes(normalizePropertyName(query)))
    .slice(0, 20);
  ...
  function add(name: string) {
    ...
    if (canonical && !selected.has(normalizePropertyName(canonical))) onChange([...values, canonical]);
    setSearch("");
    setOpen(false);
  }
    ...
    <div className="space-y-2">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button type="button" variant="outline" role="combobox" aria-expanded={open} className="h-11 w-full justify-between font-normal">
            <span className="truncate text-muted-foreground">Eigenschaft suchen oder neu anlegen…</span>
            <ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
          <Command shouldFilter={false}>
            <CommandInput value={search} onValueChange={setSearch} placeholder="Eigenschaft suchen…" />
      ...
      {values.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {values.map((value) => (
            <span key={normalizePropertyName(value)} className="inline-flex items-center gap-1 rounded-full border border-border bg-muted/60 px-2.5 py-1 text-xs font-medium text-foreground">
              {value}
              <Button type="button" variant="ghost" size="icon" className="h-5 w-5" aria-label={`${value} entfernen`} onClick={() => onChange(values.filter((item) => normalizePropertyName(item) !== normalizePropertyName(value)))}>
                <X className="h-3 w-3" />
              </Button>
            </span>
          ))}
