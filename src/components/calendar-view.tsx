...
import { calendarQuery, inspectionCalendarQuery, inspectionWarningDaysQuery } from "@/lib/queries";
import { useIdentity } from "@/hooks/use-identity";
import { cn } from "@/lib/utils";
import {
  DEFAULT_INSPECTION_WARNING_DAYS,
  INSPECTION_DUE_LABELS,
  inspectionStatus,
  type DueState,
} from "@/lib/due-dates";

/**
 * Gemeinsame Kalenderoberfläche für den Termin- und den Prüfkalender.
 * Beide Ansichten teilen Layout, Navigation, Typografie und Verhalten —
 * es gibt bewusst nur eine Kalenderimplementierung.
 */

export type CalendarMode = "planning" | "inspections";
...
type CalendarEvent = {
  ...
  machineId: string;
  kind: EventKind;
  /** Prüfzustand — nur bei Prüfterminen gesetzt. */
  due?: DueState | null;
  start: number;
  end: number;
  /** Gerätename */
...
/** Dezente Zustandsvarianten für Prüftermine — gleiche Statusfarben wie sonst. */
const INSPECTION_TONES: Record<DueState, { bar: string; dot: string }> = {
  overdue: {
    bar: "bg-destructive/12 text-destructive ring-1 ring-inset ring-destructive/25 hover:bg-destructive/20",
    dot: "bg-destructive",
  },
  today: {
    bar: "bg-status-maintenance/18 text-status-maintenance ring-1 ring-inset ring-status-maintenance/25 hover:bg-status-maintenance/28",
...
function toneFor(event: CalendarEvent) {
  ...
  if (event.kind === "inspection" && event.due) {
    return { ...base, ...INSPECTION_TONES[event.due] };
  }
  return base;
}
...
export function CalendarView({
  ...
  const windowDays = warningDays.data ?? DEFAULT_INSPECTION_WARNING_DAYS;

  const planning = useQuery({
    ...calendarQuery(identity.userId, identity.canManage, fromISO, toISO),
    enabled: mode === "planning" && (identity.canManage || !!identity.userId),
  });
  const inspections = useQuery({
    ...inspectionCalendarQuery(fromISO, toISO),
    enabled: mode === "inspections",
  });

  const source = mode === "planning" ? planning : inspections;

  const events = useMemo<CalendarEvent[]>(() => {
    const list: CalendarEvent[] = [];

    if (mode === "inspections") {
      for (const m of inspections.data ?? []) {
        if (!m.next_inspection_date) continue;
        const day = fromDateOnly(m.next_inspection_date);
        const due = inspectionStatus(m, undefined, windowDays);
        list.push({
          id: `insp-${m.id}`,
          machineId: m.id,
          kind: "inspection",
          ...
          detail: due ? INSPECTION_DUE_LABELS[due] : "Prüfung geplant",
    ...
    for (const m of data.inspections) {
      ...
      list.push({
        ...
        machineId: m.id,
        kind: "inspection",
        due: inspectionStatus({ inspection_required: true, next_inspection_date: m.next_inspection_date }, undefined, windowDays) ?? "upcoming",
        start: day.getTime(),
        end: day.getTime(),
        title: m.name,
        assetCode: m.asset_code,
    ...
    return list.sort((a, b) => a.start - b.start);
  }, [mode, planning.data, inspections.data, windowDays]);
  ...
  return (
    <>
      ...
      {source.isError ? (
        ...
        <AgendaView days={[cursor]} eventsOn={eventsOn} mode={mode} />
      ) : view === "week" ? (
        <AgendaView
...
function Legend({ mode }: { mode: CalendarMode }) {
  const items: Array<[string, string]> =
    mode === "inspections"
      ? [
          ["bg-destructive", "Überfällig"],
          ["bg-status-maintenance", "Heute fällig"],
          ["bg-primary", "Anstehend"],
        ]
      : [
  ...
  if (sections.length === 0) {
    return (
      <EmptyState
        title={
          mode === "inspections"
            ? "Keine Prüfungen in diesem Zeitraum."
            ...
            >
              ...
              {dayEvents.slice(0, 2).map((e) => (
                <EventBar
                  key={e.id}
                  event={e}
                  label={e.assetCode ? `${e.title} – ${e.assetCode}` : e.title}
                />
              ))}
              {dayEvents.length > 2 ? (
                <button
